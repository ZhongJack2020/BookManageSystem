...
<script>
  export default {
    ...
    methods: {
      async findBook() {
        const res = await this.$http.get(`http://localhost:8833/api/adminGetBooks?book_name=${this.bookName}`)
      ...
      async addBook() {
        ...
        const res = await this.$http.post('http://localhost:8833/api/adminAddBook', submit)
        if (res.status === 200) {
