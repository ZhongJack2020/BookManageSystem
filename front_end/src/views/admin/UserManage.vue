...
<script>
  export default {
    ...
    methods: {
      async findUser() {
        const res = await this.$http.get(`http://localhost:8833/api/adminGetUsers?stu_id=${this.userName}`)
      ...
      async resetPass(row) {
        ...
        const res = await this.$http.post('http://localhost:8833/api/resetPassword',submit)
        if(res.status === 200) {
